...
pub enum SurfaceDiff {
    ...
    // TODO: rename to `AlternateScreen`?
    /// The view of the terminal when the alternate screen is active, eg `vim`, etc.
    Screen(ScreenDiff),
}
...
/// "primary screen".
///
/// This is the most common output from the terminal. Even though it's called the "scrollback",
/// it's still the main interactive view, we just mostly are seeing the bottom of the scrollback
/// where the current prompt is.
...
/// A "screen" is a view of the so-called "alternate screen", that which you see when in "full
/// screen" apps, like `htop`, `vim`, etc.
#[derive(Clone, Debug, Default)]
#[non_exhaustive]
...
impl std::fmt::Debug for CompleteSurface {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let info = match self {
            ...
            Self::Screen(surface) => ("screen", surface),
        };
        write!(
            f,
            "Complete {} surface: {}x{}",
...
#[derive(Clone, Debug)]
#[non_exhaustive]
/// All the possible kinds of output possible, whether they're diffs or entire snapshots.
pub enum Output {
...
pub enum SurfaceKind {
    ...
    Scrollback,
    // TODO: rename to `AlternateScreen`?
    /// The so-called "alternate screen".
    Screen,
}
...
impl crate::shadow_terminal::ShadowTerminal {
    ...
    pub(crate) fn build_current_output(
        ...
        let is_diff_possible = self.last_sent.pty_size == (tty_size.cols, tty_size.rows);
    ...
    fn build_complete_surface(
        ...
        let complete_surface = match kind {
            ...
            SurfaceKind::Screen => {
                ...
                surface.add_changes(changes);
                tracing::warn!("{}", surface.screen_chars_to_string());
                CompleteSurface::Screen(surface)
            }
        };
